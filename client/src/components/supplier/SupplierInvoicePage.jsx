...
const SupplierInvoicePage = () => {
    let { getAllInvoices, adoptInvoice, changeStatus } = useSupplier();
    let { user } = useUser();
    ...
    const onConfirm = (id) => {
        adoptInvoice(id).then(() => {
            setAllInvoice(prevInvoice =>
                prevInvoice.map(invoiceItem =>
                    invoiceItem.id === id ? { ...invoiceItem, isAdopted: true } : invoiceItem
                )
            );
            api.success({
                message: 'Внимание!',
    ...
    const changeSelectElement = (record, value) => {
        changeStatus(record.id, value).then(() => {
            setAllInvoice(prevInvoice =>
                prevInvoice.map(invoiceItem =>
                    invoiceItem.id === record.id
                        ? { ...invoiceItem, deliveryOrderStatusId: value }
                        : invoiceItem
                )
            );
            api.success({
    ...
    const column = [
        ...
        },
        {
            ...
            key: 'invoiceFile',
            render: (text) => (
                <a href={`${process.env.REACT_APP_API_URL}/xlsx/${text}`} download>
                    {text}
                </a>
            ),
        },
        ...
        },
        {
            ...
            render: (record) => (
                <Space size="large" style={{ display: "flex", flexFlow: "column" }}>
                    {record.isAdopted ? (
                        <Select
                            defaultValue={record.deliveryOrderStatusId}
                            onChange={(value) => {
                                changeSelectElement(record, value);
                            }}
                        >
